...
Scale the badge to fit your layout using the `size` prop, with support for `sm`, `md`, and `lg`.
...
Customize the badge's visual tone with the `color` prop, supporting a wide range of semantic and vibrant options like `primary`, `error`, and `success`.
...
Enhance context by adding icons before or after your text using the `prefixIcon` and `suffixIcon` props.
