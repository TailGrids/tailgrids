...
Achieve a minimal visual footprint for tight layouts or secondary content by setting the `variant` prop to `compact`.
...
The default `sideLayout` displays both text labels and icons for side navigation buttons, providing maximum clarity.
...
Prioritize text labels for previous and next buttons by setting the `sideLayout` to `label`. This layout automatically adjusts for mobile devices.
...
Use purely visual markers for navigation by setting the `sideLayout` prop to `icon`.
