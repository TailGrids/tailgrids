...
<CodeBlockCard
  ...
  lang="tsx"
  fileName="components/ui/button-group.tsx"
/>
</Accordion>
</Accordions>
...
Style the entire group using the `variant` prop with `primary` or `secondary` themes.
...
The `size` prop allows you to adjust the scale of all buttons within the group simultaneously, supporting `sm`, `md`, and `lg` options.
