...
<CodeBlockCard
  ...
  lang="svg"
  fileName="components/ui/spinner/default.svg"
/>

<CodeBlockCard
  snippet={getFileContent("/src/registry/core/spinner/dotted.svg")}
  lang="svg"
  fileName="components/ui/spinner/dotted.svg"
/>

<CodeBlockCard
  snippet={getFileContent("/src/registry/core/spinner/dotted-round.svg")}
  lang="svg"
  fileName="components/ui/spinner/dotted-round.svg"
/>
...
Choose from three visual styles—`default`, `dotted`, and `dotted-round`—using the `type` prop to match your design language.
...
Scale your loading indicator from `sm` to `xxl` using the `size` prop to perfectly fit any UI context, from inline text to full-page loaders.
...
Enhance user experience by integrating spinners within buttons using the `sm` size to indicate active processing or pending actions.
