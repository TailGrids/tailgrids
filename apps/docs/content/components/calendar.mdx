...
The Calendar component is built on `react-aria-components` and uses date utilities from `@internationalized/date`.

```npm
npm i react-aria-components @internationalized/date @tailgrids/icons class-variance-authority
```

Copy the component code into your project. The registry exports `Calendar`, `CalendarHeader`, `NavButton`, `CalendarYearPicker`, `CalendarGrid`, `CalendarGridHeader`, `CalendarGridBody`, and `CalendarCell`.

<CodeBlockCard
  snippet={getFileContent("/src/registry/core/calendar.tsx")}
  lang="tsx"
...
The Calendar component fully supports internationalization. Use the `I18nProvider` wrapper to localize the calendar for different regions, languages, and calendar systems. See [here](https://react-aria.adobe.com/Calendar#international-calendars) for more information.
