...
<CodeBlockCard
  ...
  lang="tsx"
  fileName="components/ui/link.tsx"
/>
</Accordion>
</Accordions>
...
Customize the link's visual weight using the `variant` prop, with options for `primary` and `dark` styles.
...
Adjust the link's scale to match your typography using the `size` prop, offering both `sm` and `md` options.
...
Configure links for external navigation by applying standard `target="_blank"` and `rel` attributes.
