...
Customize the tab list's visual tone using the `variant` prop, with support for both `default` and `minimal` styles.
...
Improve navigation clarity by adding an `icon` directly to your `TabTrigger` components for a more intuitive interface.
...
Display real-time counts or status indicators next to tab labels by using the `badge` prop on the `TabTrigger`.
...
Switch to a standard horizontal tab arrangement by setting the `direction` prop to `horizontal` on the `TabRoot`.
