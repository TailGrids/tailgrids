...
The Range Calendar component is built on `react-aria-components` and uses date utilities from `@internationalized/date`.

```npm
npm i react-aria-components @internationalized/date @tailgrids/icons class-variance-authority
```

Copy the component code into your project. The registry exports `RangeCalendar`, `RangeCalendarHeader`, `RangeNavButton`, `RangeCalendarYearPicker`, `RangeCalendarGrid`, `RangeCalendarGridHeader`, `RangeCalendarGridBody`, and `RangeCalendarCell`.

<CodeBlockCard
  snippet={getFileContent("/src/registry/core/range-calendar.tsx")}
  lang="tsx"
  fileName="range-calendar.tsx"
  showLineNumbers
/>
</Accordion>
</Accordions>
