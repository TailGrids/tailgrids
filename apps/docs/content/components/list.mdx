...
Streamline your list's appearance by toggling the `hideDividers` prop to remove horizontal rules between items.
...
Highlight the currently selected item visually and semantically across your navigation or menus by applying the `data-active` attribute.
