...
Scale the checkbox to fit your UI using the `size` prop, offering both `sm` and `md` options.
...
Provide clear context for your selection by adding descriptive text using the `label` prop.
