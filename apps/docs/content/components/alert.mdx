...
Copy and paste the following component code into your project. The component includes `Alert`, `AlertIndicator`, `AlertContent`, `AlertTitle`, and `AlertDescription`.
...
The alert body. Renders as a `<p>` by default and can hold any rich content (links, lists, etc.).
