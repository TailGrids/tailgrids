...
Adjust the scale of your switch to match your layout with the `size` prop, offering `sm` and `md` options.
...
Provide clear context for binary choices by adding an optional `label` to your toggle component.
...
Manage the toggle's state programmatically using the `checked` and `onChange` props for precise interactive control.
