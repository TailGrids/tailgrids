...
Precision-position your tooltip content relative to its trigger using the `placement` prop, with support for all four directions.
...
Use custom elements or components as tooltip activators by applying the `asChild` prop to the `TooltipTrigger`.
...
Programmatically manage the tooltip's visibility or integrate it into complex UI flows using the `open` and `onOpenChange` props.
