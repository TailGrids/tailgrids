...
<CodeBlockCard
  ...
  lang="tsx"
  fileName="components/ui/breadcrumbs.tsx"
/>
</Accordion>
</Accordions>
...
Enhance navigation context by adding an `icon` to individual items within the `items` array.
...
Customize the path separator using the `dividerType` prop, with support for `slash`, `chevron`, and `dot` styles.
