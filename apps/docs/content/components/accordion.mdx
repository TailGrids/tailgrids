...
Set `variant="style_one"` on `AccordionRoot` to use the default style.
...
Use `className` on `AccordionRoot`, `AccordionItem`, and `AccordionTrigger` to build a custom look.
