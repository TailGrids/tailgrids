...
Display a sleek, linear progress indicator focused purely on visual feedback of completion state.
...
Provide explicit feedback on the completeness of a task by toggling the `withLabel` prop to show the percentage.
...
Customize the appearance of the progress bar using `className` prop.
